        ...
        <div class="col-md-12">
            <div class="box box-primary">
                ...
                {{ Form::open(array(
		                            'method'=> 'POST',
		                            'class' => '',
                                    'route' => ['admin.packageDuration.addsubmit'],
                                    'name'  => 'addPackageDurationForm',
                                    'id'    => 'addPackageDurationForm',
                                    'files' => true,
		                            'novalidate' => true)) }}
                    <div class="box-body">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                @if (count($packageList))
                                    @foreach ($packageList as $package)
                                        <option value="{{$package->id}}" @if($package->id == old('package_id') ) selected="selected" @endif>{{$package->title}}</option>
                                    @endforeach
                                @endif
                                    </select>
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                @if (count($packagePeriodList))
                                    @foreach ($packagePeriodList as $packagePeriod)
                                        <option value="{{$packagePeriod->id}}" @if($packagePeriod->id == old('package_period_id') ) selected="selected" @endif>{{$packagePeriod->title}}</option>
                                    @endforeach
                                @endif
                                    </select>
                                </div>
